...
def perform_extraction(file_path: str, url: str) -> None:
    ...
    try:
        ...
        with open(file_path, "rb") as f:
            files_payload = {**multipart_data, "0": (file_path, f, "application/pdf")}
...
def main():
    parser = argparse.ArgumentParser(description="Doctracer Workflow: Tabular Data Extraction Sample")
    parser.add_argument("file_path", help="Path to the PDF file to process")
    parser.add_argument("--url", default=DEFAULT_URL, help=f"GraphQL API URL (default: {DEFAULT_URL})")
